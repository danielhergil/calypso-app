...
public abstract class BaseEncoder implements EncoderCallback {
  ...
  protected void getDataFromEncoder() throws IllegalStateException {
    ...
    // BEFORE dequeueInputBuffer
    Log.d(TAG, "BaseEncoder: queue size before input = " + queue.size());
    ...
    // AFTER you enqueue input
    Log.d(TAG, "BaseEncoder: queue size after input = " + queue.size());
    ...
    while (running) {
      ...
      if (outBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
        ...
        long dt = (System.nanoTime() - t0) / 1000;  // µs it took to process this buffer
        Log.d(TAG, String.format(
                "BaseEncoder: processed frame flags=0x%X size=%d pts=%d in %dµs, queue now=%d",
                bufferInfo.flags, bufferInfo.size, bufferInfo.presentationTimeUs, dt, queue.size()));
      } else {
        break;
      }
    }
