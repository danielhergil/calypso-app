...
class CameraCalypsoApiManager(context: Context) : CameraDevice.StateCallback() {
    ...
    var whiteBalance: Int
        get() {
            val builderInputSurface = this.builderInputSurface ?: return CaptureRequest.CONTROL_AWB_MODE_AUTO
            return builderInputSurface.secureGet(CaptureRequest.CONTROL_AWB_MODE) ?: CaptureRequest.CONTROL_AWB_MODE_AUTO
        }
        set(value) {
            val characteristics = cameraCharacteristics ?: return
            val builderInputSurface = this.builderInputSurface ?: return
            ...
            try {
                builderInputSurface.set(CaptureRequest.CONTROL_AWB_MODE, value)
                cameraCaptureSession.setRepeatingRequest(
                    builderInputSurface.build(),
                    if (faceDetectionEnabled) cb else null, null
                )
    ...
    fun setManualWhiteBalance(temperature: Float) {
        ...
        // Set color correction mode to use the provided gains/transform.
        builderInputSurface.set(CaptureRequest.COLOR_CORRECTION_MODE, CaptureRequest.COLOR_CORRECTION_MODE_TRANSFORM_MATRIX)

        // Calculate gains from the temperature value.
        val gains = calculateGainsFromTemperature(temperature)
        builderInputSurface.set(CaptureRequest.COLOR_CORRECTION_GAINS, gains)

        // Here we use an identity matrix for the color correction transform.
        // In a real implementation you might calculate a custom transform.
        // Create an identity 3x3 transform using Rationals:
        ...
        builderInputSurface.set(CaptureRequest.COLOR_CORRECTION_TRANSFORM, identityMatrix)
    ...
    /**
     * A simple (approximate) function to calculate white balance gains from a temperature.
     *
     * This function maps a temperature (in Kelvin, clamped to 2000K–8000K) to a set of gains.
     * The RggbChannelVector contains (red, greenEven, greenOdd, blue) multipliers.
     *
     * **Note:** This is an approximate mapping for demonstration purposes.
     */
    private fun calculateGainsFromTemperature(temperature: Float): RggbChannelVector {
        ...
        // For demonstration, we create a simple linear mapping:
        // At 2000K, we assume the sensor needs to boost red and reduce blue.
        // At 8000K, the opposite is assumed.
        // Green is held constant at 1.0.
        // These values are only approximate!
        val redMultiplier = 2.0f - (temp - 2000f) / 6000f * 1.5f  // decreases from ~2.0 to ~0.5
        val blueMultiplier = 0.5f + (temp - 2000f) / 6000f * 1.5f // increases from ~0.5 to ~2.0
