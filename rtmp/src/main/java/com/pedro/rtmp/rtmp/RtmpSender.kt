  ...
  override suspend fun onRun() {
    while (scope.isActive && running) {
      val error = runCatching {
        // 1) Measure how long we wait on queue.take()
        val tTakeStart = System.nanoTime()
        val mediaFrame = runInterruptible { queue.take() }
        val tTakeMs = (System.nanoTime() - tTakeStart) / 1_000_000
        Log.d(TAG, "RtmpSender: dequeued frame after ${tTakeMs}ms, remainingInQueue=${queue.getSize()}")
        getFlvPacket(mediaFrame) { flvPacket ->
          ...
          if (flvPacket.type == FlvType.VIDEO) {
            ...
            socket?.let { socket ->
              ...
              if (isEnableLogs) {
                Log.i(TAG, "wrote Video packet moreno, size $size")
              }
            }
          } else {
            audioFramesSent++
          ...
          val tSendMs = (System.nanoTime() - tSendStart) / 1_000_000
          Log.d(TAG, "RtmpSender: sent ${flvPacket.type} packet, size=${size} bytes in ${tSendMs}ms; queueSize=${queue.getSize()}")
          bytesSend += size
        }
      }.exceptionOrNull()
      if (error != null) {
